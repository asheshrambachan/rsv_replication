...
# probability predictions produced by 01_data_construction/07_evaluate_predictors.R.
#
# Input:  data/clean/forest/uganda_sim_Y_bin80_w_pred.csv
# Output: figures/forest/stability/
# =============================================================================
...
# columns (R0, R1, ...) produced by 07_evaluate_predictors.R.
# -----------------------------------------------------------------------------

outcome <- "Y_bin80"

in_path <- file.path("data/clean/forest", paste0("uganda_sim_", outcome, "_w_pred.csv"))
DT      <- fread(in_path)
...
for (So_col in c("S_o_02km", "S_o_05km", "S_o_10km")) {
  ...
  for (y_val in y_levels) {
    ...
    out_path <- file.path("figures/forest/stability", 
