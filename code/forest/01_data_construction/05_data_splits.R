...
# Forest — Data Splits
#
# Takes the merged forest cover + MOSAIKS dataset and produces two analysis-
# ready splits used downstream:
#
#   <country>_train.csv  — predictor-training half (01_data_construction/)
#   <country>_sim.csv    — simulation half (03_sims_noexpoutcomes/,
#                          04_sims_expoutcomes/)
#
# Three outcomes are constructed from forest cover:
#   Y_bin80      — binary: 1 if forest cover exceeds the 80th percentile of
#                  the PES imagery region (used in noexpoutcomes simulations)
#   Y_discrete   — 5-bin discretisation based on quintiles of the imagery
#                  region distribution; bins represented by midpoints
#   Y_continuous — raw forest cover proportion (used in expoutcomes sims)
#
...
input_path   <- sprintf("data/clean/forest/%s_%s_%sdeg.csv", country, year, resolution)
imagery_path <- "data/clean/forest/shapefiles/imagery_boundary.gpkg"

out_train <- file.path("data/interim/forest", paste0(country, "_train.csv"))
out_sim   <- file.path("data/interim/forest", paste0(country, "_sim.csv"))
...
#
# Y_bin80: binary outcome used in the no-experimental-outcomes simulations.
...
#
# Y_discrete: 5-bin discretisation used in the experimental-outcomes
...
#
# Y_continuous: raw forest cover proportion, used as a continuous outcome
...
dt[, Y_bin80 := as.integer(forest_cover > tau80)]
...
dir.create("data/interim/forest", showWarnings = FALSE, recursive = TRUE)
