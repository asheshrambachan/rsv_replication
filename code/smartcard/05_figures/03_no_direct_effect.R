# =============================================================================
# Assumption 3(ii) — No Direct Effect: Density Figures
#
# Visualises f(R | S=e, D=0, Y=y) vs f(R | S=e, D=1, Y=y) for each outcome
# and level of Y. Supports the no-direct-effect assumption (D ⊥ R | S=e, Y):
# if treatment has no direct effect on the satellite signal, the two densities
...
# Output: figures/smartcard/no_direct_effect/[y_cons|y_lowinc|y_midinc]/
# =============================================================================
...
data <- read_csv("data/clean/smartcard/smartcard_data.csv",
...
for (y in c(0,1)){
  for (Y_var in c("Ycons","Ylowinc", "Ymidinc")){
    ...
    # Define aesthetics (only D0_e and D1_e appear; data is filtered to S=="e")
    group_colors <- c(
      D0_e = palette$blue,
    ...
    output_path <- sprintf("figures/smartcard/no_direct_effect/%s/smartcard_r_dist_%s%s.pdf", y_var_snake, y_var_snake, y)
