...
base_map <- function(data, fill, guide, legend.position.inside = c(0.73,0.234)) {
  ...
  # Load shapefiles
  state <- st_read("data/clean/smartcard/shapefiles/state.gpkg", quiet=T)
  districts <- st_read("data/clean/smartcard/shapefiles/districts.gpkg", quiet=T)
  shrids <- st_read("data/clean/smartcard/shapefiles/shrids.gpkg", quiet=T)
...
# Output directory
output_dir <- "figures/smartcard/maps"
