...
#   data/raw/smartcard/shrug-pc11state-poly-shp/state.shp
#   data/raw/smartcard/shrug-pc11dist-poly-shp/district.shp
#   data/raw/smartcard/shrug-shrid-poly-gpkg/shrid2_open.gpkg
#
# Outputs:
#   data/clean/smartcard/shapefiles/state.gpkg
#   data/clean/smartcard/shapefiles/districts.gpkg
#   data/clean/smartcard/shapefiles/shrids.gpkg
# =============================================================================
...
dir.create("data/clean/smartcard/shapefiles", recursive = TRUE, showWarnings = FALSE)

# -----------------------------------------------------------------------------
# 1. State Boundary
# -----------------------------------------------------------------------------

state <- st_read("data/raw/smartcard/shrug-pc11state-poly-shp/state.shp", quiet = TRUE) %>%
  filter(s_name == "Andhra Pradesh")

st_write(state, "data/clean/smartcard/shapefiles/state.gpkg",
...
districts <- st_read("data/raw/smartcard/shrug-pc11dist-poly-shp/district.shp", quiet = TRUE) %>%
  filter(pc11_s_id %in% state$pc11_s_id) %>%
  mutate(d_name = recode(d_name, !!!district_recode))

st_write(districts, "data/clean/smartcard/shapefiles/districts.gpkg",
...
shrid_ids <- read_csv("data/clean/smartcard/smartcard_data.csv",
                      col_select = shrid2) %>%
  pull(shrid2) %>%
  unique()

shrids <- st_read("data/raw/smartcard/shrug-shrid-poly-gpkg/shrid2_open.gpkg", quiet = TRUE) %>%
  filter(shrid2 %in% shrid_ids)

st_write(shrids, "data/clean/smartcard/shapefiles/shrids.gpkg",
