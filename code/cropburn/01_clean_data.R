...
output_path <- "data/processed/cropburn/districts/districts.shp"
...
villages <- st_read("data/raw/shrug-pc11-village-poly-shp/village_modified.shp", quiet=T) %>%
  filter(pc11_d_id %in% districts$pc11_d_id) %>%
  select(-mdds_og)

output_path <- "data/processed/cropburn/villages/villages.shp"
dir.create(dirname(output_path), recursive = TRUE, showWarnings = FALSE)
