...
run_one <- function(Y_var) {
  ...
  # Export Results
  output_path <- sprintf("data/poverty/interim/rsv_realD_synthS_%s_gate_sample.rds", Y_var)
  dir.create(dirname(output_path), recursive = T, showWarnings = F)
  saveRDS(out, output_path)

  # Return Summary
  sprintf("[RSV]  GATE realD realS %s; coef = %.3f (%.3f); denominator = %.3f (%.3f) → saved to %s\n", Y_var, out$coef, out$se, out$denominator, out$denominator_se, output_path)
}
