...
cat("=== PPI Observational Simulations ===\n")
cat("Using", n_cores, "cores for parallel processing\n\n")

# Create output directory
outcome   <- "Ybin"
estimator <- "ppio"
out_dir   <- file.path("data/interim/ugandaforestcover", paste0("sims_expoutcomes_", outcome), estimator)
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

compute_ppio_estimate <- function(Y, R, D, S_e, S_o, y_levels) {
  ...
  D[S_o_only == 1] <- 0 # replace NA with 0 for S_o_only == 1
  D_o <- D[S_o == 1]
  error_o <- error[S_o == 1]
  delta_D <- mean(error_o[D_o == 1]) - mean(error_o[D_o == 0])
  
  gamma_D + delta_D
}

bootstrap_se_ppio <- function(Y, R, D, S_e, S_o, B, y_levels, 
...
run_single_config <- function(config_idx, params, out_dir) {
  ...
  run_one <- function(b) {
    ...
    coefficients <- compute_ppio_estimate(Y, R, D, S_e, S_o, y_levels)
    se <- bootstrap_se_ppio(Y, R, D, S_e, S_o, B, y_levels, seed = NULL, num.threads = 1)
...
params <- readRDS(file.path("data/interim/ugandaforestcover", paste0("sims_expoutcomes_", outcome), "params.Rds"))
