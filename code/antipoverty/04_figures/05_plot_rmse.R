...
est_theme <- data.frame(
  levels = c("true", "rsv", "surrogate"),
  labels = c("Benchmark", "RSV", "Common practice"),
  colors = c("black", "#43739D", "#ff6666"),
  shapes = c(17, 19, 23)
)

data <- read.csv("data/clean/antipoverty/results_simulations_cons_buffer_holdout.csv")  %>%
  mutate(
...
for (n in n_list){
  fig <- data %>% 
    ...
    ggplot(aes(x = ate, y = rmse, group=estimator, color=estimator, fill=estimator, shape=estimator)) + 
    ...
    labs(
      ...
      y = ifelse(is_first(n, n_list), "Root mean square error", ""),
      color = NULL,
      shape = NULL,
      fill = NULL
    ) +
    scale_y_continuous(limits = c(0, 0.5), minor_breaks = seq(0, 0.5, 0.02)) +
  ...
  output_path <- sprintf("output/figures/antipoverty_rmse/n%s.jpeg", n)
