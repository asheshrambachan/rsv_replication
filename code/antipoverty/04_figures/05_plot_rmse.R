...
data <- read.csv("data/clean/antipoverty/rsv_vs_surrogate_sameDGP.csv")  %>%
  mutate(
...
for (n in n_list){
  fig <- data %>% 
    ...
    scale_y_continuous(limits = c(0, 0.6), minor_breaks = seq(0, 1, 0.02)) +
    scale_x_continuous(limits = c(0.0, 0.6), breaks = seq(-1, 1, 0.1)) +
    theme( 
  ...
  output_path <- sprintf("output/figures/antipoverty_rmse/n%s_sameDGP.jpeg", n)
