...
data <- read.csv("data/clean/antipoverty/estimates_correct_seed42.csv") %>%
...
for (Y_name in c("Ycons", "Y05k", "Y10k")){
  fig <- data %>%
    filter(Y==Y_name) %>%
    ...
    labs(
      y = ifelse(Y_name=="Ycons", "Denominator (Relevance)", ""),
      shape=NULL,
      color=NULL
    ) +
    scale_color_manual(
  ...
  output_path <- sprintf("output/figures/antipoverty_relevance/%s_correct_seed42.jpeg", Y_name)
