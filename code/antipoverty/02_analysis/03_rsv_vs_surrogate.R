...
for(n in c(1000, 2000, 3000)){
  for(tau in seq(from = 0, to = 0.5, by = 0.1)){
    ...
    output_path <- sprintf("data/raw/antipoverty/simulations/rsv_nfold5_B%s_n%s_tau%.1f.rds", B, n, tau)
    ...
    output_path <- sprintf("data/raw/antipoverty/simulations/surrogate_nfold5_B%s_n%s_tau%.1f.rds", B, n, tau)
    dir.create(dirname(output_path), recursive = TRUE, showWarnings = FALSE)
