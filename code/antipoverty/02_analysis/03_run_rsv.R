...
YS_list <- grep(pattern = "^Y(05k|10k|cons)_S(real|synth)$", colnames(data), value = TRUE)
for (YS_name in YS_list) {
  Y <- data[[YS_name]]
  
  set.seed(123)
  out <- rsv_fun(
    X_train = X, D_train = D, Y_train = Y,
    X_test = X, D_test = D, Y_test = Y, 
  ...
  output_path <- sprintf("data/clean/antipoverty/simulations/rsv_%s.rds", YS_name)
